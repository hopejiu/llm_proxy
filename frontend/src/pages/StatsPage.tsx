...
function Card({ l, n, d, tr, cost, hideCost }: any) {
  const row1 = [
    { k: "total_input_tokens", lab: "输入" },
    { k: "total_output_tokens", lab: "输出" },
    { k: "total_cached_tokens", lab: "缓存" },
  ];
  const row2 = [
    { k: "uncached_input", lab: "无缓存输入" },
  ...
  return (
    <div className="stat-card">
      ...
      <div className="flex items-baseline gap-1.5 mb-1">
        ...
        <span className="text-xs text-[#9C94B0]">词元{tr}</span>
      </div>
      {!hideCost && cost != null && (
        <div className="mb-2">
      ...
      <div className="space-y-2 mt-3 pt-3 border-t border-[#F0EBF5]">
...
export default function StatsPage() {
  ...
  const [hideCost, setHideCost] = useLocalStorage('stats_hide_cost', false);
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      if (modelFilter) {
        ...
        logger.info("Provider模式加载", {
          sp, msCount: ms?.length, dailyCount: d?.length,
  ...
  const sT = [
    { name: "输入词元", type: "line" as const, smooth: true, data: filled.map((d: any) => d.total_input_tokens || 0), itemStyle: { color: "#7C3AED" }, lineStyle: { color: "#7C3AED", width: 2 }, areaStyle: { color: { type: "linear" as const, x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "rgba(124,58,237,0.15)" }, { offset: 1, color: "rgba(124,58,237,0)" }] } } },
    { name: "输出词元", type: "line" as const, smooth: true, data: filled.map((d: any) => d.total_output_tokens || 0), itemStyle: { color: "#A78BFA" }, lineStyle: { color: "#A78BFA", width: 2 }, areaStyle: { color: { type: "linear" as const, x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "rgba(167,139,250,0.15)" }, { offset: 1, color: "rgba(167,139,250,0)" }] } } },
    { name: "缓存词元", type: "line" as const, smooth: true, data: filled.map((d: any) => d.total_cached_tokens || 0), itemStyle: { color: "#10B981" }, lineStyle: { color: "#10B981", width: 2 }, areaStyle: { color: { type: "linear" as const, x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "rgba(16,185,129,0.15)" }, { offset: 1, color: "rgba(16,185,129,0)" }] } } },
  ];
  const trendOpt = {
    tooltip: { trigger: "axis" as const, axisPointer: { type: "cross" as const, crossStyle: { color: "#9C94B0" } } },
    legend: { data: dim === "tokens" ? ["输入词元", "输出词元", "缓存词元"] : ["请求数"], textStyle: { color: "#6B6580", fontSize: 11 }, bottom: 0 },
  ...
  const hOpt = (() => {
    ...
    return { tooltip: { trigger: "axis" as const, axisPointer: { type: "shadow" as const } }, legend: { data: ["词元消耗", "请求次数"], textStyle: { color: "#6B6580", fontSize: 11 }, bottom: 0 }, grid: { left: "3%", right: "4%", bottom: "14%", top: "8%", containLabel: true }, xAxis: { type: "category" as const, data: hl, axisLabel: { color: "#9C94B0", fontSize: 10 }, axisLine: { lineStyle: { color: "#EDE9FE" } }, axisTick: { lineStyle: { color: "#EDE9FE" } } }, yAxis: [{ type: "value" as const, name: "词元", position: "left" as const, axisLabel: { color: "#9C94B0", fontSize: 10, formatter: (v: number) => v >= 10000 ? (v / 10000).toFixed(0) + "万" : String(v) }, splitLine: { lineStyle: { color: "#F0EBF5" } }, nameTextStyle: { color: "#6B6580", fontSize: 10 } }, { type: "value" as const, name: "请求次数", position: "right" as const, axisLabel: { color: "#9C94B0", fontSize: 10, formatter: (v: number) => v >= 1000 ? (v / 1000).toFixed(0) + "k" : String(v) }, splitLine: { show: false }, nameTextStyle: { color: "#6B6580", fontSize: 10 } }], series: [{ name: "词元消耗", type: "bar" as const, yAxisIndex: 0, data: td, itemStyle: { color: { type: "linear" as const, x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "#7C3AED" }, { offset: 1, color: "#A78BFA" }] }, borderRadius: [4, 4, 0, 0] as [number, number, number, number] } }, { name: "请求次数", type: "line" as const, yAxisIndex: 1, data: rd, smooth: true, itemStyle: { color: "#F59E0B" }, lineStyle: { color: "#F59E0B", width: 2 } }] };
  })();
  ...
  const sOpt = (() => {
    ...
    return { tooltip: { trigger: "axis", axisPointer: { type: "shadow" } }, legend: { data: legendData, textStyle: { color: "#6B6580", fontSize: 11 }, bottom: 0 }, grid: { left: "3%", right: "4%", bottom: "14%", top: "8%", containLabel: true }, xAxis: { type: "category", data: hl, axisLabel: { color: "#9C94B0", fontSize: 10 }, axisLine: { lineStyle: { color: "#EDE9FE" } }, axisTick: { lineStyle: { color: "#EDE9FE" } } }, yAxis: { type: "value", name: "词元", axisLabel: { color: "#9C94B0", fontSize: 10, formatter: (v: number) => v >= 10000 ? (v / 10000).toFixed(0) + "万" : String(v) }, splitLine: { lineStyle: { color: "#F0EBF5" } }, nameTextStyle: { color: "#6B6580", fontSize: 10 } }, series };
  })();
  ...
  return (<div className="page-container">
    <div className="page-header">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setHideCost(!hideCost)} className="btn-ghost p-1.5" title={hideCost ? "显示花费" : "隐藏花费"}>
          {hideCost ? (
          ...
          <div className="flex items-center gap-1 bg-brand-50 rounded-lg p-0.5">
            {(["today", "week", "total"] as const).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-3 py-1 text-xs font-medium rounded-md transition-all duration-150 ${period === p ? "bg-white text-brand-700 shadow-sm" : "text-[#6B6580] hover:text-brand-600"
                  }`}
              >
          ...
          <div className="table-wrap" ref={mbTableRef} onScroll={onMbTableScroll}>
            <table className="table-base text-xs">
              <thead><tr className="border-b border-[#F0EBF5]">
                ...
                <th className="table-th text-right">输入</th>
                <th className="table-th text-right">无缓存输入</th>
                <th className="table-th text-right">输出</th>
                <th className="table-th text-right">缓存</th>
                {!hideCost && <th className="table-th text-right text-emerald-600">输入费</th>}
                {!hideCost && <th className="table-th text-right text-emerald-600">输出费</th>}
                {!hideCost && <th className="table-th text-right text-emerald-600">缓存费</th>}
                {!hideCost && <th className="table-th text-right font-semibold text-brand-700">总花费</th>}
              </tr></thead>
              <tbody>{modelBreakdown.map((item: any, i: number) => (
                <tr key={i} className="table-tr">
                  ...
                  <td className="table-td text-right font-mono text-xs">{item.total_input_tokens != null ? (item.total_input_tokens - (item.total_cached_tokens || 0)).toLocaleString() : "-"}</td>
                  ...
                  {!hideCost && <td className="table-td text-right font-mono text-xs text-emerald-600">{fmtYuan(item.inputCost)}</td>}
                  {!hideCost && <td className="table-td text-right font-mono text-xs text-emerald-600">{fmtYuan(item.outputCost)}</td>}
                  {!hideCost && <td className="table-td text-right font-mono text-xs text-emerald-600">{fmtYuan(item.cacheCost)}</td>}
                  {!hideCost && <td className="table-td text-right font-mono text-xs font-bold text-brand-700">{fmtYuan(item.cost)}</td>}
                </tr>
              ))}</tbody>
            </table>
          </div>
      ...
      <div className="table-wrap" ref={ddTableRef} onScroll={onDdTableScroll}>
        <table className="table-base text-xs">
          <thead><tr className="border-b border-[#F0EBF5]">
            ...
            <th className="table-th text-right hidden sm:table-cell">输入</th><th className="table-th text-right hidden sm:table-cell">无缓存输入</th>
            <th className="table-th text-right hidden sm:table-cell">输出</th>
            <th className="table-th text-right hidden sm:table-cell text-emerald-600">缓存</th>            <th className="table-th text-right hidden sm:table-cell text-emerald-600">命中率</th><th className="table-th text-right font-semibold text-brand-700">总计</th>
            {!hideCost && <th className="table-th text-right text-emerald-600">花费</th>}
          </tr></thead>
          <tbody>{dailyWithCost.length === 0 ? <tr><td colSpan={hideCost ? 8 : 9} className="text-center py-12 text-[#9C94B0]">暂无数据</td></tr>
            : [...dailyWithCost].reverse().map((s: any, i: number) => (<tr key={i} className="table-tr">
              ...
              <td className="table-td text-right font-mono text-xs hidden sm:table-cell">{s.total_input_tokens != null ? (s.total_input_tokens - (s.total_cached_tokens || 0)).toLocaleString() : "-"}</td>
    ...
    <div className="section-card">
      ...
      <RecentRequestsTable logs={enrichedRecent} showTps showCost={!hideCost} emptyText="暂无请求记录" />
    </div>
  </div>);
}
